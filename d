...
local GameName = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name
                if (tick() - Tick) > 0.15 then
                    watermark_title.Text = string.format("status = private user, uid = 1, ping = %s,  fps = %u, | "..GameName, library.shared.ping, library.shared.fps)
                    window.watermark:UpdateSize()
                    --
                    Tick = tick()
                end
    ...
    function pages:PlayerList(info)
        ...
        function playerList:getThumbnail(userId)
            local req = syn and syn.request or request
            local userData = req({
                Url = "https://thumbnails.roblox.com/v1/users/avatar-headshot?size=48x48&format=png&userIds=" .. tostring(userId),
                Method = "GET"
            }).Body

            local decoded = http:JSONDecode(userData);

            if not decoded.errors then
        ...
        function playerList:Refresh(Relation)
            ...
            if Selection then
                if lastselection ~= Selection then
                    ...
                    local imageData = game:HttpGet(playerList:getThumbnail(Selection[1].UserId));
                    --
                    --print(imagedata)
        ...
        --
        function playerList:GetPlayerData(player)
            for i,v in pairs(playerList.players) do
                if v[1] == player then
                    return v;
                end
            end
    ...
    function sections:Dropdown(info)
        ...
        utility:LoadImage(dropdown__gradient, "gradient")
        --
    ...
    function sections:Multibox(info)
        ...
        utility:LoadImage(multibox__gradient, "gradient")
        --
    ...
    function sections:Colorpicker(info)
        ...
        utility:LoadImage(colorpicker__gradient, "gradient")
        --
        function colorpicker:Set(color, transp_val)
            if typeof(color) == "table" then
                colorpicker.current = color
    ...
    function sections:Listbox(info)
        ...
        --
        utility:LoadImage(listbox_gradient, "gradient")
        --
    ...
    function sections:ConfigBox(info)
        ...
        --
        utility:LoadImage(configLoader_gradient, "gradient")
        --
